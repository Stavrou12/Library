...
function req_show_data() {
    if (document.getElementById("forminf").hidden === false) {
        $('#forminf').hide();
        document.getElementById("forminf").hidden = true;
        $('#save-butt').hide();
        return 0;
    }
    var xhr = new XMLHttpRequest();
    xhr.onload = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            document.getElementById("txtPaso").value = data.student_id;
            ...
            document.getElementById("txtTypeUserUni").value = data.university;
            document.getElementById("txtTmhma").value = data.department;
            document.getElementById("txtDatePasoEnd").value = data.student_id_to_date;
            document.getElementById("txtDatePasoStart").value = data.student_id_from_date;
            document.getElementById("txtTypeUserStudent").value = data.student_type;
        } else if (xhr.status !== 200) {
            alert('Request failed. Returned status of ' + xhr.status);
        }
    };
    $('#forminf').show();
    document.getElementById("forminf").hidden = false;
    $('#save-butt').show();
    xhr.open('GET', 'servlet_userdata');
    xhr.send();
...
function update(){
    let myForm = document.getElementById("forminf");
